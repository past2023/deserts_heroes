...
(function () {
  ...
  const enemyShipArt = [new Image(), new Image()];
  enemyShipArt[0].src = 'assets/space/ships/enemies_ship01.png';
  enemyShipArt[1].src = 'assets/space/ships/enemies_ship02.png';
  ...
  function spawnEnemy(type, x, opts) {
    ...
    } else if (type === 'space_fighter') {
      base.hp = 6; base.y = opts.y !== undefined ? opts.y : 140; base.bobT = rnd(0, 6);
  ...
  function killEnemy(e, dir) {
    ...
    } else if (e.type === 'space_fighter') {
      spawnCoinAward(e.x, e.y - 18, 'heli');
      explode(e.x, e.y, 50, false, false);
      spawnDestructionFire(e.x, e.y, 0.85, 8);
      G.corpses.push({
        spaceFighter: true, vehicleWreck: true, scale: 0.9, shipIdx: e.shipIdx || 0,
        x: e.x, y: e.y + 22,
        vx: rnd(-55, 55), vy: 40, angle: 0, spin: rnd(2.5, 5),
        facing: e.facing, t: 0, life: 2.1, nextBoom: 0.22, boomCount: 0,
      });
    } else if (e.type === 'tank') {
  ...
  function updateEnemies(dt) {
    ...
    for (const e of G.enemies) {
      ...
      switch (e.type) {
        ...
        case 'space_fighter': {
          e.bobT += dt;
          const targetX = p.x + Math.sin(e.t * 0.5) * 200;
          const targetY = 110 + Math.sin(e.bobT * 1.6) * 30;
          e.x += clamp(targetX - e.x, -160, 160) * dt;
          e.y += clamp(targetY - e.y, -80, 80) * dt;
          e.facing = dx > 0 ? 1 : -1;
          if (e.fireT <= 0 && !p.dead) {
            const ang = Math.atan2((p.y - 28) - e.y, dx);
            for (let i = -1; i <= 1; i++) {
              G.eBullets.push({
                x: e.x, y: e.y + 12,
                vx: Math.cos(ang + i * 0.14) * 340, vy: Math.sin(ang + i * 0.14) * 340,
                life: 2.2, style: 'enemyGunship', phase: rnd(0, Math.PI * 2), trailT: 0,
              });
            }
            SFX.enemyShot('enemyGunship');
            muzzleBlast(e.x, e.y + 12, 5, Math.cos(ang), Math.sin(ang), 'enemyGunship', 0.9);
            e.fireT = rnd(1.4, 2.2);
          }
          break;
        }
  ...
  function drawCriticalText(g, x, y, time) {
    ...
    const criticalSize = criticalLabel.length > 13 ? 8 : criticalLabel.length > 9 ? 10 : 12;
    g.font = 'italic bold ' + criticalSize + 'px "Press Start 2P", "Courier New", monospace'; g.textAlign = 'center';
  ...
  function drawEnemies(g, camX) {
    for (const e of G.enemies) {
      ...
      if (isInfantry(e.type)) {
        ...
        const img = enemyShipArt[e.shipIdx || 0];
        if (img && img.naturalWidth > 0) {
          const sw = 120, sh = 56;
          g.save(); g.translate(Math.round(sx), Math.round(e.y));
          if (e.facing < 0) g.scale(-1, 1);
          g.shadowColor = '#68efff'; g.shadowBlur = 10 + Math.sin(e.t * 6) * 3;
          if (e.flash) g.filter = 'brightness(0) invert(1)';
  ...
  function updateParticles(dt) {
    for (const pa of G.particles) {
      ...
      if (pa.kind === 'coin' && pa.y > Level.GROUND - pa.size * 0.55 && pa.vy > 0) {
        pa.y = Level.GROUND - pa.size * 0.55;
        if (!pa.landed) {
          pa.landed = true;
          pa.vy *= -0.22;
          pa.vx *= 0.66;
          pa.grav = 760;
          pa.life = Math.max(pa.life, pa.t + 1.05);
        } else {
          pa.vy = 0;
          pa.vx *= 0.82;
          pa.grav = 0;
    ...
    for (const c of G.corpses) {
      ...
      c.angle += c.spin * dt;
      const floorY = (c.heli || c.spaceFighter) ? Level.GROUND : Level.GROUND + 6;
      if (c.y > floorY) {
      ...
      if ((c.heli || c.spaceFighter) && c.y >= floorY - 1) {
  ...
  function drawParticles(g, camX) {
    ...
    for (const c of G.corpses) {
      if (c.tank) {
        ...
        g.restore();
      } else if (c.spaceFighter) {
        g.save();
        g.globalAlpha = Math.max(0, 1 - c.t / c.life * 0.6);
        g.translate(c.x - camX, c.y);
        g.rotate(c.angle * 0.3);
        const img = enemyShipArt[c.shipIdx || 0];
        if (img && img.naturalWidth > 0) {
          const sw = 120, sh = 56;
          if (c.t > 1.0) {
            ...
            g.shadowBlur = 15;
            g.filter = 'brightness(0.5) sepia(1)';
            if (c.facing < 0) g.scale(-1, 1);
            g.drawImage(img, -sw / 2, -sh / 2, sw, sh);
            g.filter = 'none';
          } else {
