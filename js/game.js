...
(function () {
  ...
  const IS_PORTAL_MODE = !!(window.IS_PORTAL || (typeof PortalLevel !== 'undefined' && Level === PortalLevel));
  ...
  function startGame(mode, characterId) {
    ...
    } else if (IS_PORTAL_MODE) {
      MusicTracks.play('space');
    } else {
  ...
  function drawTutorialBoard(g, camX) {
    ...
    if (mounted) {
      ...
      if (pulse>0.4) {
        g.fillStyle = '#000'; g.font = 'bold 7px "Press Start 2P", "Courier New", monospace'; g.textAlign='center';
  ...
  function update(dt) {
    ...
    // Portal level exit: when player reaches the exit portal, return to Level 1
    if (IS_PORTAL_MODE && G.mode === 'arcade' && G.player && !G.player.dead &&
        Level.PORTAL_EXIT_X && Math.abs(G.player.x - Level.PORTAL_EXIT_X) < 42) {
      let saved = null;
      ...
      if (saved) {
        ...
        sessionStorage.setItem('dh_portal_return', JSON.stringify(saved));
      }
      SFX.stopMusic();
      window.location.href = 'level1.html?resumePortal=1';
      return;
    }
  ...
  function text(str, x, y, size, color, align, bold) {
    g.font = (bold === false ? '' : 'bold ') + Math.round(size * 0.65) + 'px "Press Start 2P", "Courier New", monospace';
  ...
  function drawWrapped(value, x, y, maxWidth, lineHeight, size, color) {
    ...
    const lines = [];
    let line = '';
    g.font = 'bold ' + Math.round(size * 0.65) + 'px "Press Start 2P", "Courier New", monospace';
  ...
  function drawCharacterSelect() {
    ...
    g.font = 'bold 12px "Press Start 2P", "Courier New", monospace';
    ...
    if (bioLines.length > 2) { g.font = 'bold 8px "Press Start 2P", "Courier New", monospace'; bioLines = wrapSelectInfo(bioStr, 690, 3); }
    if (bioLines.length > 2) { g.font = 'bold 7px "Press Start 2P", "Courier New", monospace'; bioLines = wrapSelectInfo(bioStr, 720, 3); }
    ...
    for (let i = 0; i < stats.length; i++) {
      ...
      g.font = 'bold 11px "Press Start 2P", "Courier New", monospace';
      g.textAlign = 'left';
      g.fillStyle = '#fff3c7';
    ...
    if (Characters.roster.length > 1) {
      ...
      g.font = 'bold 22px "Press Start 2P", "Courier New", monospace'; g.textAlign = 'center'; g.fillStyle = accent.border;
  ...
  function render() {
    ...
    if (G.state === 'menu') drawMenu();
      ...
      drawWorld();
      drawScreenFX();
      if (!IS_PORTAL_MODE) ExtremeForeground.draw(g);
  ...
  } else if (window.IS_PORTAL) {
    // Portal level: restore saved state from Level 1 and start in arcade mode
    let saved = null;
    try { saved = JSON.parse(sessionStorage.getItem('dh_portal_return') || 'null'); } catch (e) {}
    const pc = (saved && Characters.isValid(saved.characterId)) ? saved.characterId : initialCharacter;
    setTimeout(function(){
      startGame('arcade', pc);
      G.intro = null;
      // Spawn player on the first platform (no ground in space)
      if (Level.platforms && Level.platforms.length > 0) {
        const fp = Level.platforms[0];
