...
(function () {
  ...
  function drawCreditSlide(local, now) {
    ...
    g.font = 'bold 16px "Press Start 2P", "Courier New", monospace'; g.textAlign = 'center';
  ...
  function drawStorySlide(local, now) {
    ...
    for(let i=0;i<42;i++){
      ...
      if(i%7===0){ g.font='5px "Press Start 2P","Courier New",monospace'; g.fillText(Math.random()>0.5?'1':'0', x, y); }
    }
    g.restore();
    ...
    for(let i=0;i<lines.length;i++){
      ...
      const y=startY + i*62;
      g.font='bold 10px "Press Start 2P", "Courier New", monospace';
  ...
  function drawSlideFour(local, now) {
    ...
    // Sand wind streaks (shared system).
    drawSandWind(now, 1.0);

    // Heat wave distortion: vertical sine displacement on the lower third.
    g.save();
    g.globalAlpha=0.08; g.globalCompositeOperation='lighter';
    for(let i=0;i<6;i++){
      const hy=350+i*28;
      const offset=Math.sin(now*0.002+i*1.7)*3;
      g.fillStyle='rgba(255,200,120,0.12)';
      g.fillRect(0,hy+offset,W,2);
    }
    g.restore();
