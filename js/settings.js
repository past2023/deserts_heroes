  ...
  function draw(g, VW, VH) {
    ...
    for (const row of layout.rows) {
      ...
      if (item.kind === 'slider') {
        ...
        txt(g, Input.label(codes[0]), 440, y, 15, selectedRow ? '#ffe28a' : '#caa86a');
        txt(g, Input.label(codes[1]), 600, y, 15, selectedRow ? '#ffe28a' : '#7a6a4a');
        if (selectedRow && !Input.isCapturing()) {
          txt(g, tr('settings.bindingHelp'), 820, y, 10, '#777', 'right');
        }
      }
    }
    g.restore();
